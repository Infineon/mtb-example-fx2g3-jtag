...
* \version 1.0
*
* \details  This is the source code for the USB JTAG  Application Example
*           for ModusToolbox.
*
...
void Cy_Jtag_RespChannelDataWire_ISR (void)
{
    ...
    Cy_USB_AppClearDmaInterrupt(&appCtxt, APP_RESPONSE_IN_EP, CY_USB_ENDP_DIR_IN);

    /* The current data buffer has been consumed. Move to another buffer if available. */
    Cy_Jtag_AppRespSendCompletion(&appCtxt);
}
...
{
    /* Clear the interrupt first. */
    Cy_USB_AppClearDmaInterrupt(&appCtxt, APP_CMD_OUT_EP, CY_USB_ENDP_DIR_OUT);

    /* The current data buffer has been consumed. Move to another buffer if available. */
    Cy_Jtag_AppCmdRecvCompletion(&appCtxt);
}
...
int main (void)
{
    ...
    Cy_SysLib_Delay(500);
    Cy_Debug_AddToLog(1, "********** FX2G3: JTAG Application ********** \r\n");
